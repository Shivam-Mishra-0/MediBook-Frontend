...
import { providerAPI, appointmentAPI, authAPI, formatDate, formatTime } from '../../utils/api';
    ...
    const load = async () => {
      try {
        ...
        await Promise.allSettled(
          provList.slice(0, 30).map(async (p) => {
            const r = await appointmentAPI.getByProvider(p.providerId);
            const list = r.data || [];

            for (const a of list) {
              let patientName = `Patient #${a.patientId}`;

              try {
                const res = await authAPI.getProfile(a.patientId);
                patientName = res.data.fullName;
              } catch (e) {
                console.error("Failed to fetch patient", a.patientId);
              }
